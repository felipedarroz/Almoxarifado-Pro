...
export const AnalyticsView: React.FC<AnalyticsViewProps> = ({ deliveries }) => {
    ...
    // 1. Processar dados para Gráfico de Tendência (Entregas por Dia)
    const trendData = useMemo(() => {
        const grouped: Record<string, number> = {};
        deliveries.forEach(d => {
            // Pega apenas a data YYYY-MM-DD
            const date = d.issueDate.split('T')[0];
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                    <h3 className="text-sm font-semibold text-slate-700 mb-4 flex items-center gap-2">
                        <TrendingUp size={16} /> Evolução de Entregas (30 dias)
                    </h3>
                    <div className="h-[300px] w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={trendData}>
                                ...
                                <Tooltip />
                                <Area type="monotone" dataKey="entregas" stroke="#8884d8" fillOpacity={1} fill="url(#colorEntregas)" />
                            </AreaChart>
                        </ResponsiveContainer>
                    </div>
                </div>
